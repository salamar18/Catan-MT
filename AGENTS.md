...
bot.js (entry)
  ...
  └─ WS framereceived (id 130, type 4 / 91)
        ├─ wsGameState.ts     → cache full state / merge diffs; derive GameState
        │                        (1v1 opponent hand = 19 - bank - myHand)
        └─ readGameState.flushGameStateLog() → overwrite logs/game-state.log
  └─ decideMove.ts / performAction.ts → strategy + clicks [STUB, not wired from bot.js]
```
...
- **Players / Hand / Deck**: resource counts, dev cards, knights, VP; `inferredResources` + `resourceCardCount` per player
- **Phase / Dice**: current game phase, last dice roll
- **resourcesInferenceFresh**: trust inferred opponent hands only when hand-size checks pass

### Opponent resource inference (1v1)

Colonist base supply is **19 of each** resource. Opponent card *types* are hidden in WS (`0` placeholders), so for 1v1:

`opponent[r] = 19 - bank[r] - myHand[r]`

`resourcesInferenceFresh` is true only when:
1. `sum(myHand) === my resourceCardCount`
2. `sum(opponent.inferredResources) === opponent.resourceCardCount`
3. no negative residuals before clamp

Strategy code should ignore inferred hands when stale (transient WS splits).

### Current development status

- **Complete**: Chrome CDP setup, lobby automation (`startGame.ts`), WS game-state cache, event-driven logging, 1v1 bank-inferred opponent hands + stale gate
- **TODO**: strategy logic in `decideMove.ts`, action execution in `performAction.ts`, wire play loop from `bot.js`
...
`logs/game-state.log` is overwritten on each game WS message (`type:4` full state, `type:91` diff). Heartbeats (`id:"136"`) do not trigger writes. There is no poll interval.
